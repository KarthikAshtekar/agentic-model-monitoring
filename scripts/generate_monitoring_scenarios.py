...
def main() -> int:
    ...
    manifests = (
        generate_all_scenarios(overwrite=args.overwrite)
        if args.all
        else [generate_scenario(args.scenario, overwrite=args.overwrite)]
    )
    print(f"Generated {len(manifests)} monitoring scenario(s).")
    for manifest in manifests:
        candidates = ",".join(manifest.expected_incident_candidates)
        print(
            f"  {manifest.scenario_name}: rows={manifest.generated_sample_count}, "
